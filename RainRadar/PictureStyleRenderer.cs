...
public class PictureStyleRenderer : ISkiaStyleRenderer
{
    public bool Draw(SKCanvas canvas, Viewport viewport, ILayer layer, IFeature feature, IStyle style, IRenderCache renderCache, long currentIteration)
    {
        try
        {
            var pictureFeature = feature as PictureFeature;
            var picture = pictureFeature?.Picture;

            var opacity = (float)(layer.Opacity * style.Opacity);
            var paint = new SKPaint();

            paint.Color = paint.Color.WithAlpha((byte)Math.Round(opacity * 255.0f));

            if (picture == null)
                return false;

            if (!(style is PictureStyle pictureStyle))
                return false;
            ...
            if (viewport.IsRotated())
            {
                ...
                canvas.DrawPicture(picture.Data!, paint);
            ...
            else
            {
                ...
                canvas.Scale(destination.Width / picture.Data!.CullRect.Width);
